...
# ============================================================================
# NAVIGATION AGENT FOLLOWER AI WITH DOOM-STYLE ANIMATIONS - Godot 4.5
# ============================================================================
# This script enables any CharacterBody3D to follow a target (typically the 
# player) using NavigationAgent3D for pathfinding around obstacles, with
# automatic sprite-based directional animations like classic DOOM games.
#
# SETUP REQUIREMENTS:
# 1. Attach this script to a CharacterBody3D node
# 2. Add a NavigationAgent3D as a child node
# 3. Add an AnimatedSprite3D as a child node with these animations:
#    - "idle"
#    - "walk_forward"
#    - "walk_backward"
#    - "walk_left"
#    - "walk_right"
# 4. Ensure your scene has a NavigationRegion3D with baked navigation mesh
# 5. Tag your player node with the group specified in player_group
# ============================================================================
...
@export_group("Random Movement Behavior")
## Enable random evasive movement to make NPC harder to hit
@export var enable_random_movement: bool = false

## Time between random movement intervals (seconds)
@export_range(0.1, 10.0, 0.5) var random_movement_interval: float = 3.0

## Random variance in interval timing (prevents predictability)
@export_range(0.0, 3.0, 0.1) var interval_variance: float = 1.0

## Maximum distance for random movement from current position
@export_range(2.0, 20.0, 1.0) var random_movement_radius: float = 8.0

## Timeout for reaching random point before returning to following
@export_range(0.5, 10.0, 0.5) var random_movement_timeout: float = 4.0

## Probability of random movement triggering each interval (0-1)
@export_range(0.0, 1.0, 0.05) var random_movement_chance: float = 0.7
...
# Random movement state tracking
enum MovementState { FOLLOWING, RANDOM_MOVING }
var current_movement_state: MovementState = MovementState.FOLLOWING
var random_movement_timer: float = 0.0
var random_movement_timeout_timer: float = 0.0
var random_destination: Vector3 = Vector3.ZERO
var next_random_interval: float = 0.0
...
func _ready() -> void:
	...
	# Initialize random movement timer
	if enable_random_movement:
		_set_next_random_interval()
...
func _physics_process(delta: float) -> void:
	...
	# Update random movement behavior if enabled
	if enable_random_movement:
	...
	if nav_agent.is_navigation_finished():
		...
		# If we reached a random destination, return to following
		if current_movement_state == MovementState.RANDOM_MOVING:
			_return_to_following()
...
func _update_random_movement(delta: float) -> void:
	match current_movement_state:
		MovementState.FOLLOWING:
			# Count down to next random movement attempt
			random_movement_timer += delta
			
			if random_movement_timer >= next_random_interval:
				# Time to potentially trigger random movement
				if randf() <= random_movement_chance:
					_start_random_movement()
				else:
					# Failed chance, reset timer
					_set_next_random_interval()
					random_movement_timer = 0.0
		
		MovementState.RANDOM_MOVING:
			# Track timeout while moving to random position
			random_movement_timeout_timer += delta
			
			# Return to following if timeout exceeded
			if random_movement_timeout_timer >= random_movement_timeout:
				if debug_mode:
					print(name, " random movement timed out, returning to following")
				_return_to_following()

func _start_random_movement() -> void:
	# Generate a random point within radius
	var random_offset = Vector3(
		randf_range(-random_movement_radius, random_movement_radius),
		0.0,
		randf_range(-random_movement_radius, random_movement_radius)
	)
	
	random_destination = global_position + random_offset
	
	# Set navigation target to random destination
	nav_agent.target_position = random_destination
	
	# Update state
	current_movement_state = MovementState.RANDOM_MOVING
	random_movement_timeout_timer = 0.0
	
	if debug_mode:
		print(name, " starting random movement to: ", random_destination)

func _return_to_following() -> void:
	current_movement_state = MovementState.FOLLOWING
	random_movement_timer = 0.0
	_set_next_random_interval()
	
	if debug_mode:
		print(name, " returning to following mode")

func _set_next_random_interval() -> void:
	# Calculate next interval with variance
	var variance = randf_range(-interval_variance, interval_variance)
	next_random_interval = random_movement_interval + variance
	next_random_interval = max(0.1, next_random_interval)  # Ensure positive
...
func _update_navigation_target() -> void:
	...
	# Only update to player position if in FOLLOWING mode
	if current_movement_state == MovementState.FOLLOWING:
